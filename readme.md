...
- **Evidence factor (E)** decides how much of the Rhythm and Momentum *points*
  ...
  With `k` scheduled sessions in the report period, the curve is a stretched
  exponential normalised to reach exactly 1 at seven sessions:
  `E = min(1, (1 - exp(-(k / 1.6) ** 0.65)) / (1 - exp(-(7 / 1.6) ** 0.65)))`.
  The exponent below 1 front-loads the curve so the first repeats are worth the
  most evidence and each later one a little less — the same intuition as the
  `min(1, k / 3)` confidence inside R and M, but smooth instead of kinked. Seven
  is the saturation point because it is the Rhythm observation window (Momentum
  uses six): past it, neither metric takes in new observations, so there is
  nothing left for evidence to withhold. E only scales the contribution: the
  reported R and M values are exactly as measured, `E` can never add points (so
  an untouched record still scores 0), and it runs 0.56 at one session, 0.74 at
  two, 0.84 at three, 0.90 at four, 0.95 at five, 0.98 at six, and 1.0 from
  seven on, so established habits keep the long-term scoring behaviour exactly.
  A perfectly kept habit therefore scores about 70 at 1/1, 79 at 2/2, 87 at 3/3,
  90 at 5/5, and 92.5 from 7/7 onward.
